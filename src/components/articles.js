...
export default class Articles extends Component {
	...
	componentWillMount() {
		return new Promise((resolve) => {
			fetch('http://blog.etherial.fr/articles').then((result) => result.json()).then((json) => {
					...
					<li className="home-article radial-out">
						<Link to={`/articles/${article.id}`} className="link">
							{article.title.length > 75 ? (
								<h3>{article.title.slice(0, 75)}...</h3>
							) : (
								<h3>{article.title}</h3>
							)}
							<p>
							...
							{article.created_at !== article.updated_at ? (
								<Moment
									...
									format="[Last update:] dddd, MMM Do YYYY, [at] HH:mm"
								/>
							) : null}
							<p>Category: {article.ArticleCategory.name}</p>
						</Link>
					</li>
				))}
			</ul>
